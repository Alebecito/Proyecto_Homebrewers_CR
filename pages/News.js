      ...
      { method: 'GET', }).then((response) => response.json()).then(async (responseJson) => {
        var temporalData = [];
          for (var i = 0; i < responseJson.length; i++) {
            ...
            temporalData.push({ id: responseJson[i].publicacionNoticiaGUID, title: responseJson[i].titulo, time: this.formatDate(responseJson[i].fecha), image: responseJson[i].fotoPublicacionNoticia, description: responseJson[i].cuerpo, teGusta:this.state.relacionAuxiliar, likes: responseJson[i].cantidadDeLikes, comentarios: responseJson[i].cantidadDeComentarios })
